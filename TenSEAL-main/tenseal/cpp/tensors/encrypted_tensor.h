...
class EncryptedTensor {
   public:
    ...
    /*
    方法检查当前TenSEALContext的 auto_mod_switch 设置是否为真，
    以及ct 和 other 是否具有相同的 parms_id
    parms_id 是加密参数的唯一标识符，它用于表示加密对象使用的参数集合（包括模数等）
    如果auto_mod_switch 为假或者 ct 和 other 具有相同的 parms_id，方法直接返回。
    否则，调用当前对象的 set_to_same_mod 方法，将 ct 调整为与 other 具有相同的模数。
    */
    ...
    */
    ...
    void set_to_same_mod(T& other, Ciphertext& ct) {
        /*
        用于获取一个对象的链索引（chain index）。
        链索引表示在模数切换操作中，对象在链中的位置，与其参数集合（模数）相关联。
        */
