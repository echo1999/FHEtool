...
// double generateRandomNumber_bgv_serve(int integerDigits, int decimalDigits) {
//     // 生成整数部分
//     int integerPart = 0;
...
// 读取数组和参数
int readArraysAndParamsFromFile_bgv(const std::string& filename,
...
void bgv_multiply_and_reliearize_modswitch_business_serve(Ciphertext &encrypted1, Ciphertext &encrypted2, Evaluator &evaluator, Decryptor &decryptor,RelinKeys &relin_keys) {
...
uint64_t bgv_hexStringToUint64_serve(const std::string &hexString)
{
    std::stringstream ss;
...
void bgv_mul(size_t poly_modulus_degree, int n, int data_decimal, int mul_decimal, int mul_times, double data_randomNumber[], double mul_randomNumber[], double real[]){
    ...
    for(int i = 0; i < n; ++i){
        ...
        for(int j = 0; j < mul_times; ++j){
            // cout << "j:" << j << endl;
            try{
            bgv_multiply_and_reliearize_modswitch_business_serve(data_encrypted, mul_encrypted, evaluator, decryptor, relin_keys);
    ...
    //    cout << "乘法结束" << endl;
        ...
        uint64_t decrypted_unit64 = bgv_hexStringToUint64_serve(decrypted.to_string());
...
void bgv_serve(){
    ...
    // for (int i = 0; i < n; ++i) {
    //     // 生成满足条件的随机数
    //     data_randomNumber[i] = generateRandomNumber_bgv_serve(data_integer, data_decimal);
    //     // cout << std::fixed << std::setprecision(data_decimal+1) << "randomNumber:" << data_randomNumber << endl;
    //     mul_randomNumber[i] = generateRandomNumber_bgv_serve(mul_integer, mul_decimal);
    ...
    int n = readArraysAndParamsFromFile_bgv("/echo-project/ckks/data.txt", data_randomNumber, mul_randomNumber, real, data_integer, data_decimal, mul_integer, mul_decimal, mul_times);

    bgv_mul(4096, n, data_decimal, mul_decimal, mul_times, data_randomNumber, mul_randomNumber, real);
    bgv_mul(8192, n, data_decimal, mul_decimal, mul_times, data_randomNumber, mul_randomNumber, real);
    bgv_mul(16384, n, data_decimal, mul_decimal, mul_times, data_randomNumber, mul_randomNumber, real);
    bgv_mul(32768, n, data_decimal, mul_decimal, mul_times, data_randomNumber, mul_randomNumber, real);
