...
//    mybgv();
    ...
    // ckks_business();
    // bgv_business();
    // bgv_serve(11,2,1,0,2);
    // cout << "DATA_INTEGER:" << DATA_INTEGER;

    if (METHOD == 1){
        // bfv_serve(DATA_INTEGER, DATA_DECIMAL, MUL_INTEGER, MUL_DECIMAL, MUL_TIMES);
        bgv_serve();
    }else if(METHOD == 2){
        // bgv_serve(DATA_INTEGER, DATA_DECIMAL, MUL_INTEGER, MUL_DECIMAL, MUL_TIMES);
        bfv_serve();
    }else if(METHOD == 3){
        // ckks_serve(DATA_INTEGER, DATA_DECIMAL, MUL_INTEGER, MUL_DECIMAL, MUL_TIMES);
        ckks_serve();
    }else{
        cout << "error" << endl;
    }
    
    // ckks_serve(3,5,1,0,2);
