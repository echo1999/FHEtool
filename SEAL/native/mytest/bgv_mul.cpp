...
void bgv_multiply(Ciphertext &encrypted1, Ciphertext &encrypted2, Evaluator &evaluator, Decryptor &decryptor) {
...
void bgv_multiply_and_reliearize(Ciphertext &encrypted1, Ciphertext &encrypted2, Evaluator &evaluator, Decryptor &decryptor,RelinKeys &relin_keys) {
    evaluator.multiply_inplace(encrypted1, encrypted2);
...
void bgv_multiply_and_reliearize_modswitch(Ciphertext &encrypted1, Ciphertext &encrypted2, Evaluator &evaluator, Decryptor &decryptor,RelinKeys &relin_keys) {
    evaluator.multiply_inplace(encrypted1, encrypted2);
    // Relinearize the result
    evaluator.relinearize_inplace(encrypted1, relin_keys);
    // cout << "    + noise budget after relinearize: " << decryptor.invariant_noise_budget(encrypted1) << " bits"
    ...
    cout << "    + noise budget after mod switch: " << noise_budget << " bits"
    << endl;
...
//     cout << "Plaintext matrix row size: " << row_size << endl;
    ...
    for (int i = 1; i < max_multiplications; ++i) {
        ...
        bgv_multiply_and_reliearize_modswitch(encrypted_res, encrypted_x, evaluator, decryptor, relin_keys);
