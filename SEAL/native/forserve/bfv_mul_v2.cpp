...
void bfv_v2_multiply_and_reliearize_modswitch(
...
uint64_t bfv_hexStringToUint64(const std::string &hexString)
{
    std::stringstream ss;
...
void bfv_mul_v2()
{
    ...
    /*
    Batching and slot operations are the same in BFV and bfv.
    */
    ...
    for (int j = 2; j <= maxtimes; j++)
    {
        ...
        for (int i = 1; i < max_multiplications; ++i)
        {
            ...
            // bfv_multiply_and_reliearize(encrypted_res, encrypted_x, evaluator, decryptor, relin_keys);
            bfv_v2_multiply_and_reliearize_modswitch(encrypted_res, encrypted_x, evaluator, decryptor, relin_keys);
            ...
            if (noise_budget != 0)
            {
                ...
                uint64_t decrypted_uint64 = bfv_hexStringToUint64(decrypted_x.to_string());
